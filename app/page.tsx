...
export default function Dashboard() {
  ...
  const fetchMetrics = async () => {
    ...
    const { count: countTurnos } = await supabase.from('lavadero_turnos').select('*', { count: 'exact', head: true }).gte('created_at', todayIso);
  ...
  useEffect(() => {
    ...
    const turnosSub = supabase.channel('turnos_changes').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'lavadero_turnos' }, fetchMetrics).subscribe();
